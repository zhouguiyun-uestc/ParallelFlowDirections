...
void CommISend(msg_type & msg, int dest, int tag)
{
	...
	//I_Isend(msg.data(), msg.size(), MPI_BYTE, dest, tag, MPI_COMM_WORLD, &request);
...
void hostProcess(GridInfo& gridInfo, std::vector<TileInfo>& tileInfos, IObjectFactory* pIObjFactory)
{
	...
	for (int i = 0; i < tileInfos.size(); i++)
	{
		...
		//CommISend(msgs.back(), (gridInfo.grandWidth%active_consumer_limit) + 1, TagFirst);
	...
	std::cout << "maxTimecal:" << maxTimecal << ",minTimecal:" << minTimecal << ",avgTimecal:" << time_first_total.calc / active_consumer_limit << std::endl;
		...
		for (int i = 0; i < tileInfos.size(); i++)
		{
			...
			//CommSend(&tileInfos[i], pP2c, &gridInfo, (gridInfo.grandWidth %active_consumer_limit) + 1, TagSecond);
