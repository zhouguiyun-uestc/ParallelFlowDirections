...
bool Consumer::processRound2(const GridInfo& gridInfo, const TileInfo& tileInfo, IProducer2Consumer* pIProducer2Consumer)
{
	...
	LabelFlatArea(false, flowdirs, labels, flatHeight, highEdgeMask, lowEdgeMask, IdBounderyCells); //label the flat area

	if (!NoFlat)
	{
		RenewMask(tileInfo, pIProducer2Consumer,flowdirs,IdBounderyCells,highEdgeMask,lowEdgeMask,labels,flatHeight);// renew the lowmask and high mask
		D8FlowFlats(flowdirs,labels);// the flow direction of flat area
	}
	
	//std::cout << "ModifyBoundaryFlowDir" << gridRow << "," << gridCol << std::endl;
	ModifyBoundaryFlowDir(tileInfo,pIProducer2Consumer,flowdirs,labels,IdBounderyCells,lowEdgeMask,flatHeight);// renew the flow direction of  the border cell
	timer_calc.stop();
	timer_io.start();
...
{
	//initialize flow direction of DEM
	...
	for (int row = 0; row < flowdirs.getHeight(); row++)
	{
		for (int col = 0; col < flowdirs.getWidth(); col++)
		{
			...
			{
				flowdirs.at(row, col) = flowdirs.getNoDataValue();//if the elevation of the cell is nodata ,then its flow direction is 255
			}
			else
			{
				flowdirs.at(row, col) = D8FlowDir(row, col,height,width,d8Tile); // the flow direction  of the cell
			}
		}
	}
}
...
int Consumer::D8FlowDir(const int & row, const int & col,const int &height,const int &width, uint8_t d8Tile)
{
	...
	{
		if (gridRow == 0 && gridCol == 0 && row == 0 && col == 0)//  the cell is on the left top border of the DEM
		{
			return 5;
		}
		else if (gridRow == 0 && gridCol == gridWidth - 1 && row == 0 && col == width - 1)// the cell is on the right top border of the DEM
		{
			return 7;
		}
		else if (gridRow == gridHeight - 1 && gridCol == gridWidth - 1 && row == height - 1 && col == width - 1)// the cell is on the bottom right border of the cell
		{
			return 1;
		}
		else if (gridRow == gridHeight - 1 && gridCol == 0 && row == height - 1 && col == 0) // the cell is on the bottom left border of the cell
		{
			return 3;
		}
		else if (gridRow == 0 && row == 0)// the cell is  on the left border of the DEM
		{
			return 6;
		}
		else if (gridCol == gridWidth - 1 && col == width - 1)//the cell is on the right border of the DEM
		{
			return 0;
		}
		else if (gridRow == gridHeight - 1 && row == height - 1)//the cell is on the bottom border of the DEM
		{
			return 2;
		}
		else if (gridCol == 0 && col == 0)// the cell is on the left border of the DEM
		{
			return 4;
		}
	}
	...
	for (int n = 0; n <= 7; n++)
	{
		...
		if (dem.at(dem.getRow(n, row), dem.getCol(n, col)) < minimum_dem
			|| dem.at(dem.getRow(n, row), dem.getCol(n, col)) == minimum_dem
			&& flowdir >-1 && flowdir % 2 == 1 && n % 2 == 0) // if the adjacent cells  of the cell have the same minimum elevation,horizontal and vertica are prior
		{
...
void Consumer::LabelFlatArea(bool firstTime,Raster<int>& flowdirs,Raster<int>& labels,std::vector<int>&flatHeight,
	Raster<int> &highEdgeMask,Raster<int> &lowEdgeMask,std::vector<int> &IdBounderyCells)
{
	...
	if (low_edges.size() == 0 && high_edges.size() == 0 && uncertainEdges.size()==0&& flowdirs.at(0, 0) == -1)// the tile is a flat area
	{
		FlatTile = true;
	...
	else
	{
		...
		{
			if (labels.at(i->row, i->col) == 0) //label the flat
			{
	...
	for (int col = 0; col < width; col++)
	{
		for (int row = 0; row < height; row++)
		{
			...
			for (int n = 0; n <= 7; n++)
			{
				...
				if (!flowdirs.isInGrid(nRow, nCol) || flowdirs.isNoData(nRow, nCol))//flowdirs.isNoData(nRow, nCol)
				{
					continue;
				}
				if (flowdirs.at(row, col) != -1 && flowdirs.at(nRow, nCol) == -1
					&& dem.at(nRow, nCol) == dem.at(row, col)) 
				{
					low_edges.push_back(GridCell(row, col));
					break;
				}
				else if (flowdirs.at(row, col) == -1 &&
					dem.at(row, col) < dem.at(nRow, nCol))
				{
...
void Consumer::BuildAwayGradient(std::deque<GridCell> high_edges,Raster<int> &flowdirs,Raster<int>&labels,Raster<int>&highEdgeMask,std::vector<int>&flatHeight)
{
	...
	while (high_edges.size() != 1)
	{
		...
		for (int n = 0; n <= 7; n++)
		{
			...
			if (labels.at(nRow, nCol) == labels.at(row, col)
				&& flowdirs.at(nRow, nCol) ==-1 && row!=0 && row!=height-1 && col!=0 && col!=width-1 )
			{
...
void Consumer::BuildPartGraph(const int groupNumber, bool firstTime,std::vector<int>&IdBounderyCells,Raster<int> &labels,Raster<int>&lowEdgeMask, Raster<int>&highEdgeMask,Raster<int> &flowdirs, std::deque<GridCell>& uncertainEdges)
{
	int ID = 2;
	int width = labels.getWidth();
	int height = labels.getHeight();
	int pos;
	std::vector<std::vector<GridCell>> boundaryCell(groupNumber);// the set are the boundary cells top bottom left right
	...
	for (int col = 0; col < flowdirs.getWidth() - 2; col++)
	{
		if ( (labels.at(row, col) > 0 
			&& flowdirs.at(row, col) == -1)|| FlatTile==true)
		{
			if (labels.at(row, col) != labels.at(row, col + 1)
				|| flowdirs.at(row, col + 1) > -1) 
			{
	...
	//the cell of first row first col
	int col = flowdirs.getWidth() - 2; 
	if ((labels.at(row, col) > 0 
		&& flowdirs.at(row, col) == -1)
		|| FlatTile == true)
	{
		bulk_num[labels.at(row, col)].push_back(boundaryCell[labels.at(row, col)].size() + 1);
		boundaryCell[labels.at(row, col)].push_back(GridCell(row, col));
	...
	//the cell of last row last column
	...
	//the cell of last row first column
	col = 1;
	if ((labels.at(row, col) > 0 
		&& flowdirs.at(row, col) == -1)
		|| FlatTile == true)
...
void Consumer::block2block(std::vector<GridCell>& boundarySet, std::vector<int>& bulk, std::vector<int> &IdBounderyCells, Raster<int>&lowEdgeMask, Raster<int>&highEdgeMask, Raster<int>&labels)
{
	...
	int number_bulk = bulk.size() - 1;
		...
		// the distance of  internal cells
	...
	// calculate the distance of block 2block
	int maxNode_bulk = 0;
	...
	for (int source_bulk = 0; source_bulk < number_bulk; source_bulk++)
	{
		...
		if (source_bulk > maxNode_bulk)
		{
			...
			do
			{			
				int32_t diff = minimum_distances[pos_1][pos_qi] -
					minimum_distances[pos_n][pos_qi];
				if ((diff == minimum_distances[pos_1][pos_zhong] -
					minimum_distances[pos_n][pos_zhong]
					&& abs(diff) == abs(cell_n - cell_1))
					|| cell_1 + 1 == cell_n)
				{
					flag = false;
				}
...
void Consumer::bc2block(GridCell source, std::vector<int32_t> & minimum_distance, Raster<int> &labels)
{
	...
	while (this_edges.size() != 1)
	{
		...
		for (int n = 0; n <= 7; n++)
		{
			...
			int nCol = labels.getCol(n, col);
			if (labels.isInGrid(nRow, nCol)
				&& labels.at(nRow, nCol) == labels.at(row, col))
				this_edges.push_back(GridCell(nRow, nCol));
		}
	}
}
void Consumer::ExternalGraph2(std::vector<int>& bulk, int & source_bulk, int & object_bulk, const std::vector<GridCell>& boundarySet, std::vector<std::vector<int32_t>>& minimum_distances, const int cell_1, const int cell_n,std::vector<int>&IdBounderyCells,int width,int height)
{
	...
	do
	{
		int32_t diff = minimum_distances[pos_1][pos_qi] -
			minimum_distances[pos_n][pos_qi];
		int32_t diff_zhong = minimum_distances[pos_1][pos_qi] -
			minimum_distances[pos_n][pos_zhong];
		if (diff <= 0)
		{
		...
		if ((diff == minimum_distances[pos_1][pos_zhong] -
			minimum_distances[pos_n][pos_zhong]
			&& abs(diff) == abs(current_n - current_1))
			|| current_n == current_1)
		{
			flag = false;
			bool flag_s1 = true;
			...
			if (ifPreviousSource)
			{
				if (minimum_distances[pos_i][pos_qi]
		> minimum_distances[pos_i-1][pos_qi]
					&& minimum_distances[pos_i][pos_zhong]
								 > minimum_distances[pos_i - 1][pos_zhong])
				{
					flag_s1 = false;
				}
			}
			...
			if (flag_s1 && flag_sn) 
			{
				...
				{
					int object_diff =
						minimum_distances[pos_i][pos_object_current_1] -
						minimum_distances[pos_i][pos_object_current_n];
					...
					if (abs(object_diff)
						== abs(object_current_n - object_current_1)
						|| object_current_n == object_current_1) 
					{
						object_flag = false;
						...
						{
							if (minimum_distances[pos_i][pos_object_current_i]
										 > minimum_distances[pos_i][pos_object_current_i-1])
							{
								flag_o1 = false;
							}
						}
						...
						if (flag_o1 && flag_on) 
						{
							...
							{
								if (minimum_distances[pos_i][pos_object_current_i]
											 > minimum_distances[pos_i-1][pos_object_current_i])
								{
...
void Consumer::RenewMask(const TileInfo & tile, IProducer2Consumer * Ip2c,Raster<int> &flowdirs,std::vector<int> &IdBounderyCells,
	Raster<int> &highEdgeMask, Raster<int> &lowEdgeMask, Raster<int> &labels,std::vector<int> &flatHeight)
{
	...
	while (highSet.size() > 0)
	{
		...
		for (int n = 0; n <= 7; n++)
		{
			...
			if (labels.at(nRow, nCol) == labels.at(row, col)
				&& flowdirs.at(nRow, nCol) == -1 && lowEdgeMask.at(nRow, nCol) != 2
...
int8_t Consumer::D8MaskedFlowDir(const int row, const int col,Raster<int> &labels)
{
	...
	for (int n = 0; n <= 7; n++)
	{
		...
		if (flatMask.at(nRow, nCol) < minimum_elevation
			|| flatMask.at(nRow, nCol) == minimum_elevation
			&& flowdir > -1 && flowdir % 2 == 1 && n % 2 == 0)
		{
...
void Consumer::ModifyBoundaryFlowDir(const TileInfo & tileInfo,IProducer2Consumer * Ip2c,Raster<int> &flowdirs,
	Raster<int>&labels, std::vector<int> &IdBounderyCells,Raster<int>&lowEdgeMask,std::vector<int> &flatHeight)
{
	...
	if (gridCol != 0) // the tile is not on the left border
	{
		...
		for (int row = 1; row < flowdirs.getHeight() - 1; row++) //the cells belong to the left border
		{
			...
			if (IdBounderyCells[po] == 0 || lowEdgeMask.at(row, col) == 2) //find the minimum elevation
			{
				...
				{
					nRow = flowdirs.getRow(t, row);
					if (minimum_elevation > p2c->left.dem[nRow]
			...
			else
			{
				...
				for (int t = 0; t <= 2; t++)
				{
					...
					if (minimum_elevation > flatMask.at(nRow, nCol)
						|| minimum_elevation == flatMask.at(nRow, nCol)
						&& n > -1 && n % 2 == 1 && t % 2 == 0)
					{
						minimum_elevation = flatMask.at(nRow, nCol);
	...
	if (gridRow != 0)// is not the top border
	{
		...
		for (int col = 1; col < flowdirs.getWidth() - 1; col++) // the cells are on the top border
		{
			...
			if (IdBounderyCells[po]== 0 || lowEdgeMask.at(row, col) == 2)
			{
				...
				{
					nCol = flowdirs.getCol(t, col);
					if (minimum_elevation > p2c->top.dem[nCol]
	...
	if (gridRow != gridHeight - 1)
	{
		...
		for (int col = 1; col < flowdirs.getWidth() - 1; col++)
		{
			...
			if (IdBounderyCells[po] == 0 || lowEdgeMask.at(row, col) == 2)
			{
				...
				{
					nCol = flowdirs.getCol(t, col);
					if (minimum_elevation > p2c->bottom.dem[nCol]
	...
	if (gridCol != 0 && gridRow != 0 && flowdirs.at(0, 0) != 255) //is not the left top border
	{
		int col = 0, row = 0;
		int nCol, nRow, n = -1;
		int po = xy2positionNum(row, col, width, height);
		...
		else
		{
			...
			for (int t = 3; t <= 4; t++)
			{
				...
				if (dem.at(row, col) == p2c->left.dem[nRow])
				{
					...
					{
						if (minimum_elevation > 2
							|| (minimum_elevation == 2 && n > -1 && n % 2 == 1 && t % 2 == 0))
						{
							minimum_elevation = 2;
							n = t;
						}
	...
	if (gridRow != gridHeight - 1 && gridCol != gridWidth - 1 && flowdirs.at(row, col) != 255) // is not on the right bottom
	{
		...
		if (p2c->right.dem.size() == 0 || p2c->bottom_right.dem.size() == 0 || p2c->bottom.dem.size() == 0)//right || bottom
		{
			...
			else
			{
				...
				{
					nCol = flowdirs.getCol(t, col);
					if (minimun_elevation > p2c->bottom.dem[nCol]
			...
			for (int t = 4; t <= 6; t++) // 4-6
			{
				nRow = flowdirs.getRow(t, row);
				nCol = flowdirs.getCol(t, col);
				if (minimun_elevation > dem.at(nRow, nCol)
		...
		else
		{
			...
			for (int t = 4; t <= 6; t++) // 4-6
			{
				...
				if (minimun_elevation > flatMask.at(nRow, nCol)
	...
	if (gridCol != 0 && gridRow != gridHeight - 1 && flowdirs.at(row, col) != 255) // is not the left bottom border
	{
		...
		if (p2c->left.dem.size() == 0 || p2c->bottom.dem.size() == 0 || p2c->bottom_left.dem.size() == 0)// left || bottom
		{
			...
			if (p2c->bottom.dem.size() == 0)//下方的为nullTile
			{
				if (minimum_elevation > dem.NoDataValue)
				{
					minimum_elevation = dem.NoDataValue;
		...
		else
		{
			...
			nCol = flowdirs.getCol(t, col);
			nRow = flowdirs.getRow(t, row);
			if (labels.at(row, col) == labels.at(nRow, nCol) && (minimum_elevation > flatMask.at(nRow, nCol)
				|| minimum_elevation == flatMask.at(nRow, nCol)
				&& n > -1 && n % 2 == 1 && t % 2 == 0))
			{
				minimum_elevation = flatMask.at(nRow, nCol);
