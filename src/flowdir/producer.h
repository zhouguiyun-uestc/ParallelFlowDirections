...
public:
	void HandleEdge(
		std::vector<float> & elev_a,
		std::vector<float> & elev_b,
		std::vector<int> & ident_a,
		std::vector<int> & ident_b,
		std::vector<std::map < int, int>> & masterGraph_lowEdge,
		const int ident_a_offset,
		const int ident_b_offset,
