...
void Producer::process(std::vector<TileInfo>& tileInfos,Grid<std::shared_ptr<IConsumer2Producer>>& gridIConsumer2Producer)
{
	...
	for (int row = 0; row < gridHeight; row++)
	{
		for (int col = 0; col < gridWidth; col++)
		{
			...
			if (Ic2p == NULL || Ic2p==nullptr )
			{
				std::cerr << "Ic2p is null but nullTile is false" << row << "," << col << std::endl;
			}
	...
	for (int row = 0; row < gridHeight; row++)
	{
		for (int col = 0; col < gridWidth; col++)
		{
			...
			if (row > 0 && col > 0 && !((Consumer2Producer*)& *gridIConsumer2Producer.at(row - 1, col - 1))->nullTile)
			{
				...
				HandleCorner(
					c->top_elevs.front(),
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row - 1, col - 1))->bot_elevs.back(),
					c->top_ID.front(),
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row - 1, col - 1))->bot_ID.back(),
					masterLowEdgeGraph,
					c->idOffset,
					((Consumer2Producer*)gridIConsumer2Producer.at(row - 1, col - 1).get())->idOffset,
					masterHighEdgeGraph);
			...
			if (row < gridHeight - 1 && col < gridWidth - 1 && !((Consumer2Producer*)& *gridIConsumer2Producer.at(row + 1, col + 1))->nullTile)
			{
				...
				HandleCorner(c->bot_elevs.back(),
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row + 1, col + 1))->top_elevs.front(),
					c->bot_ID.back(),
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row + 1, col + 1))->top_ID.front(),
					masterLowEdgeGraph,
					c->idOffset,
					((Consumer2Producer*)gridIConsumer2Producer.at(row + 1, col + 1).get())->idOffset,
					masterHighEdgeGraph);
			...
			if (row > 0 && col < gridWidth - 1 && !((Consumer2Producer*)& *gridIConsumer2Producer.at(row - 1, col + 1))->nullTile)
			{
				...
				HandleCorner(c->top_elevs.back(),
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row - 1, col + 1))->bot_elevs.front(),
					c->top_ID.back(),
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row - 1, col + 1))->bot_ID.front(),
					masterLowEdgeGraph,
					c->idOffset,
					((Consumer2Producer*)gridIConsumer2Producer.at(row - 1, col + 1).get())->idOffset,
					masterHighEdgeGraph);
			...
			if (col > 0 && row < gridHeight - 1 && !((Consumer2Producer*)& *gridIConsumer2Producer.at(row + 1, col - 1))->nullTile)
			{
				...
				HandleCorner(c->bot_elevs.front(),
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row + 1, col - 1))->top_elevs.back(),
					c->bot_ID.front(),
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row + 1, col - 1))->top_ID.back(),
					masterLowEdgeGraph,
					c->idOffset,
					((Consumer2Producer*)gridIConsumer2Producer.at(row + 1, col - 1).get())->idOffset,
					masterHighEdgeGraph);
			...
			{
				HandleEdge(c->top_elevs,
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row - 1, col))->bot_elevs,
					c->top_ID,
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row - 1, col))->bot_ID,
					masterLowEdgeGraph,
					c->idOffset,
					((Consumer2Producer*)gridIConsumer2Producer.at(row - 1, col).get())->idOffset,
					masterHighEdgeGraph);
			}
			if (row < gridHeight - 1 && !((Consumer2Producer*)& *gridIConsumer2Producer.at(row + 1, col))->nullTile)
			{
				HandleEdge(c->bot_elevs,
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row + 1, col))->top_elevs,
					c->bot_ID,
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row + 1, col))->top_ID,
					masterLowEdgeGraph,
					c->idOffset,
					((Consumer2Producer*)gridIConsumer2Producer.at(row + 1, col).get())->idOffset,
					masterHighEdgeGraph);
			...
			{
				HandleEdge(c->left_elevs,
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row, col - 1))->right_elevs,
					c->left_ID,
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row, col - 1))->right_ID,
					masterLowEdgeGraph,
					c->idOffset,
					((Consumer2Producer*)gridIConsumer2Producer.at(row, col - 1).get())->idOffset,
					masterHighEdgeGraph);
			}
			if (col < gridWidth - 1 && !((Consumer2Producer*)& *gridIConsumer2Producer.at(row, col + 1))->nullTile)
			{
				HandleEdge(c->right_elevs,
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row, col + 1))->left_elevs,
					c->right_ID,
					((Consumer2Producer*)& *gridIConsumer2Producer.at(row, col + 1))->left_ID,
					masterLowEdgeGraph,
					c->idOffset,
					((Consumer2Producer*)gridIConsumer2Producer.at(row, col + 1).get())->idOffset,
					masterHighEdgeGraph);
...
std::shared_ptr<IProducer2Consumer> Producer::toConsumer(const TileInfo& tileInfo,Grid<std::shared_ptr<IConsumer2Producer>> gridIConsumer2Producer)
{
	...
	auto job_low = std::vector<int>(graphLow.begin() + ((Consumer2Producer*)gridIConsumer2Producer.at(gridRow, gridCol).get())->idOffset, graphLow.begin() + ((Consumer2Producer*)gridIConsumer2Producer.at(gridRow, gridCol).get())->idOffset + ((Consumer2Producer*)gridIConsumer2Producer.at(gridRow, gridCol).get())->idIncrement);
	auto job_high = std::vector<int>(graphHigh.begin() + ((Consumer2Producer*)gridIConsumer2Producer.at(gridRow, gridCol).get())->idOffset, graphHigh.begin() + ((Consumer2Producer*)gridIConsumer2Producer.at(gridRow, gridCol).get())->idOffset + ((Consumer2Producer*)gridIConsumer2Producer.at(gridRow, gridCol).get())->idIncrement);
	...
	if (gridCol > 0) //left  1
	{
		...
		left = OutBoundary(c2p->right_elevs,
			std::vector<int>(graphLow.begin() + c2p->idOffset, graphLow.begin() + c2p->idOffset +c2p->idIncrement),
			std::vector<int>(graphHigh.begin() + c2p->idOffset, graphHigh.begin() + c2p->idOffset +c2p->idIncrement),
			c2p->right_ID);
	}
	if (gridCol > 0 && gridRow > 0) // top left
	{
		...
		if (c2p->nullTile)
		{
			left_top = OutBoundary();
		}
		else
		{
			left_top = OutBoundary(std::vector<float>(1, c2p->bot_elevs.back()),
				std::vector<int>(graphLow.begin() + c2p->idOffset, graphLow.begin() + c2p->idOffset +c2p->idIncrement),
				std::vector<int>(graphHigh.begin() + c2p->idOffset, graphHigh.begin() + c2p->idOffset +c2p->idIncrement),
				std::vector<int>(1, c2p->bot_ID.back()));
		}
	...
	if (gridCol < gridWidth - 1 && gridRow>0)  // top right  
	{
		...
		if (c2p->nullTile)
		{
			right_top = OutBoundary();
		}
		else
		{
			right_top = OutBoundary(std::vector<float>(1, c2p->bot_elevs.front()),
				std::vector<int>(graphLow.begin() + c2p->idOffset, graphLow.begin() + c2p->idOffset +c2p->idIncrement),
				std::vector<int>(graphHigh.begin() + c2p->idOffset, graphHigh.begin() + c2p->idOffset +c2p->idIncrement),
				std::vector<int>(1, c2p->bot_ID.front()));
		}
	...
	{
		Consumer2Producer * c2p = (Consumer2Producer *)&*gridIConsumer2Producer.at(gridRow, gridCol + 1);
		right = OutBoundary(c2p->left_elevs,
			std::vector<int>(graphLow.begin() + c2p->idOffset, graphLow.begin() + c2p->idOffset + c2p->idIncrement),
			std::vector<int>(graphHigh.begin() + c2p->idOffset, graphHigh.begin() + c2p->idOffset + c2p->idIncrement),
			c2p->left_ID);
	}
	if (gridRow < gridHeight - 1 && gridCol < gridWidth - 1) //right bottom
	{
		...
		if (c2p->nullTile)
		{
			right_bottom = OutBoundary();
		}
		else
		{
			right_bottom = OutBoundary(std::vector<float>(1, c2p->top_elevs.front()),
				std::vector<int>(graphLow.begin() + c2p->idOffset, graphLow.begin() + c2p->idOffset +c2p->idIncrement),
				std::vector<int>(graphHigh.begin() + c2p->idOffset, graphHigh.begin() + c2p->idOffset +c2p->idIncrement),
				std::vector<int>(1, c2p->top_ID.front()));
		}
	...
	{
		Consumer2Producer * c2p = (Consumer2Producer *)&*gridIConsumer2Producer.at(gridRow + 1, gridCol);
		bottom = OutBoundary(c2p->top_elevs,
			std::vector<int>(graphLow.begin() + c2p->idOffset, graphLow.begin() + c2p->idOffset +c2p->idIncrement),
			std::vector<int>(graphHigh.begin() + c2p->idOffset, graphHigh.begin() + c2p->idOffset +c2p->idIncrement),
			c2p->top_ID);
	...
	if (gridRow < gridHeight - 1 && gridCol>0) //left bottom
	{
		...
		if (c2p->nullTile)
		{
			left_bottom = OutBoundary();
		}
		else
		{
			left_bottom = OutBoundary(std::vector<float>(1, c2p->top_elevs.back()),
				std::vector<int>(graphLow.begin() + c2p->idOffset, graphLow.begin() + c2p->idOffset +c2p->idIncrement),
				std::vector<int>(graphHigh.begin() + c2p->idOffset, graphHigh.begin() + c2p->idOffset +c2p->idIncrement),
				std::vector<int>(1, c2p->top_ID.back()));
		}
...
void Producer::HandleEdge(std::vector<float>& elev_a, std::vector<float>& elev_b, std::vector<int>& ident_a, std::vector<int>& ident_b, std::vector<std::map<int, int>>& masterGraph_lowEdge, const int ident_a_offset, const int ident_b_offset, std::vector<std::map<int, int>>& masterGraph_highEdge)
{
...
void Producer::HandleCorner(float & elev_a, float & elev_b, int ident_a, int ident_b, std::vector<std::map<int, int>>& masterGraph_lowEdge, const int ident_a_offset, const int ident_b_offset, std::vector<std::map<int, int>>& masterGraph_highEdge)
{
	bool flagLow = false;
	bool flagTrue = false;
	if (ident_a > 0) 
