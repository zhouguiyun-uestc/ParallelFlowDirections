...
template < class T > bool Raster< T >::operator==( Raster< T >& other ) {
    ...
    for ( int row = 0; row < this->height; row++ ) {
        for ( int col = 0; col < this->width; col++ ) {
            if ( isNoData( row, col ) && other.isNoData( row, col ) )
                continue;
            if ( isNoData( row, col ) && !other.isNoData( row, col ) )
                return false;
            if ( !isNoData( row, col ) && other.isNoData( row, col ) )
...
template < class T > Raster< T > Raster< T >::operator-( Raster< T >& other ) {
    Raster< T > diff;
    if ( other.getWidth() != getWidth() ) {
        std::cout << "different this->width" << std::endl;
        return diff;
